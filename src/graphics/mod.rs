...
use crate::{graphics::{texture::Texture, text::FontInfoContainer}, util::{world_to_world_screen_space, Depth, self}};
...
pub fn extract_frame(timer: f32, animation_fps: f32, frame_start: usize, frame_count: usize, loop1: bool) -> Option<usize> {
    if frame_count == 0 {
        return None
    }
    let frame = (timer * animation_fps) as usize;
    if frame < frame_count || loop1 {
        Some(frame_start + frame.rem_euclid(frame_count))
    } else {
        None
    }
}

pub fn fps_for_duration(duration: f32, frame_count: usize) -> f32 {
    if !duration.is_finite() || duration < 0.0 {
        return 0.0
    }
    frame_count as f32 / duration
}
